...
import { MyERC20, MyERC20__factory, MyERC721, MyERC721__factory, TokenSale, TokenSale__factory } from "../typechain-types";
...
describe("NFT Shop", async () => {
  ...
  beforeEach(async () => {
    ...
    tokenSaleContract = await tokenSaleContractFactory.deploy(TEST_RATIO, NFT_PRICE, paymentTokenContract.address, nftContract.address);
    // await tokenSaleContract.deployTransaction.wait(); @dev this gives the transaction receipt
    await tokenSaleContract.deployed(); // this returns contract object 
  ...
  describe("When a user purchase an ERC20 from the Token contract", async () => {
    ...
    describe("When a user burns an ERC20 at the Token contract", async () => {
      beforeEach(async () => {
        ...
        await allowTx.wait();
        const burnTx = await paymentTokenContract.connect(accounts[1]).transfer(tokenSaleContract.address, expectedBalance);
        await burnTx.wait();
      });
